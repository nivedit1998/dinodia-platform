import { MatterCommissioningSession } from '@prisma/client';
...
async function resolveFriendlyName(
  ...
  try {
    ...
    console.warn('[matter workflow] Failed to read HA state for name', { entityId, err });
  }
  return entityId;
}

async function upsertDeviceOverrides(
  session: MatterCommissioningSession,
  ha: HaConnectionLike,
  entityIds: string[]
) {
...
export async function finalizeCommissioningSuccess(
  ...
  await prisma.matterCommissioningSession.update({
    where: { id: session.id },
    data: {
  ...
  return { newDeviceIds, newEntityIds, labelWarning };
}
